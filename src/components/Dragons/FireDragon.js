...
import "./../../App.scss";
import FireDragonhead from '../../images/Dragonheads/fire.jpg';
...
class FireDragon extends Component {
...
*/
 ...
 fetchOneDragon = () => {
	    fetch("http://localhost/dragonology/server/fetchFire.php")
 ...
 fetchOneSkill = () => {
	    fetch("http://localhost/dragonology/server/fetchSkillsFire.php")
	      .then(response => response.json())
	      .then(data => {
            console.log(data);
 ...
 fetchMagic = () => {
        fetch("http://localhost/dragonology/server/fetchMagicFire.php")
          .then(response => response.json())
          .then(data => {
            console.log(data);
  ...
  fetchOneGem = () => {
	    fetch("http://localhost/dragonology/server/fetchGemFire.php")
	      .then(response => response.json())
	      .then(data => {
            console.log(data);
...
*/ 
    ...
    let fireDragonStats = this.state.DragonStats.map((stats) => {
        return ( 
    ...
    let fireDragonFactsPart1 = this.state.DragonStats.map((facts) => {
        return ( 
    ...
    let fireDragonSkills = this.state.DragonSkills.map((skill) => {
        return ( 
    ...
    let fireDragonMagic = this.state.DragonMagic.map((magic) => {
        return ( 
    ...
    let fireDragonFactsPart2 = this.state.DragonStats.map((facts) => {
        return ( 
    ...
    let fireDragonGem = this.state.DragonGems.map((gem) => {
        return ( 
            
            <div className="parchment--text" key={gem.id}>
                <div> 
                    <h1 className="title">Fire dragon´s treasured gemstone:</h1>
                    <p>{gem.Description}:</p> 
                    <p>{gem.Abilities}</p>
                    <p>{gem.Found}</p>
                </div>
    ...
    let fireDragonFactsPart3 = this.state.DragonStats.map((facts) => {
        return ( 
 ...
 return (
    ...
    <div className="fact--wrapper">
        ...
        <div className="dragonhead">
            ...
            <img alt="firedragonhead" src={FireDragonhead} />
        </div>
    ...
    <div className="fact--container">
        ...
        { fireDragonFactsPart2 }

        { fireDragonGem }

        { fireDragonFactsPart3 }
