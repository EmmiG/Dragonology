...
import "./../../App.scss";
import EarthDragonhead from '../../images/Dragonheads/earth.jpg';
...
class EarthDragon extends Component {
 ...
 fetchOneDragon = () => {
	    fetch("http://localhost/dragonology/server/fetchEarth.php")
 ...
 fetchOneSkill = () => {
	    fetch("http://localhost/dragonology/server/fetchSkillsEarth.php")
	      .then(response => response.json())
	      .then(data => {
            console.log(data);
  ...
  fetchOneGem = () => {
	    fetch("http://localhost/dragonology/server/fetchGemEarth.php")
	      .then(response => response.json())
	      .then(data => {
            console.log(data);
    ...
    fetchMagic = () => {
	    fetch("http://localhost/dragonology/server/fetchMagicEarth.php")
	      .then(response => response.json())
	      .then(data => {
            console.log(data);
...
render () {
    ...
    let earthDragonStats = this.state.DragonStats.map((stats) => {
        return ( 
        ...
        let earthDragonFactsPart1 = this.state.DragonStats.map((facts) => {
        return ( 
   ...
   let earthDragonSkills = this.state.DragonSkills.map((skill) => {
        return ( 
   ...
   let earthDragonMagic = this.state.DragonMagic.map((magic) => {
        return ( 
            ...
            let earthDragonFactsPart2 = this.state.DragonStats.map((facts) => {
        return ( 
    ...
    let earthDragonGem = this.state.DragonGems.map((gem) => {
        return ( 
            
             <div className="parchment--text" key={gem.id}>
            
            <div className="Gem--box"> 
            <h1 className="title">Earth dragon´s treasured gemstone:</h1>
            <p>{gem.Description}:</p> 
            <p>{gem.Abilities}</p>
            <p>{gem.Found}</p>
            </div>
            ...
            let earthDragonFactsPart3 = this.state.DragonStats.map((facts) => {
        return ( 
 ...
 return (
     ...
     <div className="header"></div>
            ...
            <div className="dragonhead">
                 ...
                 <img alt="earthdragonhead" src={EarthDragonhead} />
     ...
     <div className="fact--container">
         ...
         <h1 className="title">Magic:</h1>
          ...
          { earthDragonFactsPart2 }

         { earthDragonGem }

         { earthDragonFactsPart3 }
